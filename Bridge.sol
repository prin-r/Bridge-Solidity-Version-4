...
library Proof {
    struct Data {
        ...
        BlockHeaderMerkleParts.Data merkleParts;
        TMSignature.Data[] signatures;
        IBridge.RequestPacket requestPacket;
        IBridge.ResponsePacket responsePacket;
        uint256 version;
        IAVLMerklePath.Data[] merklePaths;
    }
}
...
contract Bridge is IBridge, Ownable {
    ...
    function relayAndVerify(Proof.Data memory _proof)
        public
        returns (RequestPacket memory, ResponsePacket memory)
    {
        ...
        return verifyOracleData(
            _proof.blockHeight,
            _proof.requestPacket,
            _proof.responsePacket,
            _proof.version,
            _proof.merklePaths
        );
    }
...
contract BridgeWithCache is Bridge {
    ...
    function relay(Proof.Data memory _proof) public {
        (
            RequestPacket memory req,
            ResponsePacket memory res
        ) = relayAndVerify(_proof);
...
contract SimplePriceDB {
    ...
    function setPrice(Proof.Data memory _proof) public {
        bool ok = bridge.call(abi.encodePacked(hex"a17301fe", abi.encode(_proof)));
        require(ok);
        
        require(_proof.requestPacket.oracleScriptId == 1);
        
        Obi.Data memory params = Obi.from(_proof.requestPacket.params);
        require(keccak256(abi.encodePacked(string(params.decodeBytes()))) == keccak256(abi.encodePacked("BTC")));
        
        Obi.Data memory result = Obi.from(_proof.responsePacket.result);
        px = uint256(result.decodeU64());
    }
